...
i    = 0                       # 処理論文数カウンタ
logs = []                      # 投稿ログ格納用リスト
abst_fig_md = None             # Qiitaへ画像投稿時のアップロード先URL格納用（投稿失敗時はNoneのまま）
...
for p in papers:
    ...
    pdf_save_path = download_pdf(pdf_url, PDF_DIR, pdf_name)
...
# pdf保存先および論文内画像保存先フォルダを中身もろとも削除
shutil.rmtree(PDF_DIR)
shutil.rmtree(EXT_FIG_DIR)
